...
export class SceneApp {
  ...
  private selectedFloorOpen = false;
  ...
  static async create(
    ...
    self.lighting = new LightingRig(app, builder, q);
  ...
  private setupCameraFrame() {
    ...
    cf.rendering.samples = q.taa ? 1 : q.msaa;
  ...
  private bindPointer(cb: SceneCallbacks) {
    ...
    const opts = { signal: this.pointerAbort.signal };
    let activePointers = 0;
    el.addEventListener(
      "pointerdown",
      (e) => {
        activePointers++;
        // a second finger means pinch/pan — never treat it as a tap
        this.downAt = activePointers === 1 ? { x: e.clientX, y: e.clientY, t: performance.now() } : null;
      },
      opts
    );
    const release = () => {
    ...
    el.addEventListener(
      ...
      (e) => {
        ...
        if (moved > slop || performance.now() - d.t > 600) return;
  ...
  setSelection(floor: PublicFloor | null, homeId: string | null) {
    ...
    this.selectedFloorOpen = floor != null;
  ...
  setMode(mode: SceneMode) {
    ...
    if (this.selectedFloorOpen) this.lighting.top.light!.intensity = 0.55;
  ...
  setQuality(q: Quality) {
    ...
    this.lighting.applyShadowTier(s);
  ...
  destroy() {
    ...
    const w = window as unknown as { __scene?: SceneApp };
    if (w.__scene === this) delete w.__scene;
